...
public class Solution1 {
    public boolean checkInclusion(String s1, String s2) {
        ...
        while (start <= s2Len - slideLen) {
            ...
            if (isValid(s1, s1ChCounts, s2ChCounts)) {
                return true;
            }
    ...
    private boolean isValid(String s1, int[] chS1Counts, int[] chS2Counts) {
        for (char ch : s1.toCharArray()) {
            int index = ch - 'a';
            if (chS1Counts[index] != chS2Counts[index]) {
                return false;
            }
        }
        return true;
